class AI
  def get_coord(fleet, ai)
    ...
    previous_guesses = previous_guesses_array
    if ai == true
      ...
      when 1 # if first shot hit a ship then find an adjacent shot, otherwise random
        previous_guesses[-1] ? (return find_random_adjacent(fleet, -1)) : (return find_random_shot(fleet))
      when 2 # when 2 shots have been taken
        ...
        if previous_guesses[-1] && previous_guesses[-2]
          return find_in_line_shot_one_two(fleet, -1, -2)
        elsif previous_guesses[-1] # otherwise if only previous shot hit, hit adjacent
          return find_random_adjacent(fleet, -1)
          ...
          return find_random_adjacent(fleet, -2)
        else
        ...
        if fleet.guesses.last.sunk # if you sunk a ship try a random guess
          ...
          return find_in_line_shot_one_two(fleet, -3, -1)
          ...
          return find_in_line_shot_one_two(fleet, -3, -2)
        elsif !previous_guesses[-1] && previous_guesses[-2] && previous_guesses[-3] &&
              fleet.guesses.length >= 5 && previous_guesses[-4] && !previous_guesses[-5]
              # same as before but with longer ships
          return find_in_line_shot_one_two(fleet, -4, -3)
        elsif !previous_guesses[-1] && previous_guesses[-2] && previous_guesses[-3]
              # same as before but with shorter
          return find_in_line_shot_one_two(fleet, -3, -2)
          ...
          return find_random_adjacent(fleet, -3)
        else
  ...
  def previous_guesses_array
    @fleet.guesses.map do |guess|
      guess.hit
    end
  end

  def find_in_line_shot_one_two(fleet, guess_one, guess_two)
    ...
    unless dir_in_line == nil
      possible_coords = CoordMath.get_4_coords(hit_one, 1)
  ...
  def find_random_adjacent(fleet, num_to_go_back)
